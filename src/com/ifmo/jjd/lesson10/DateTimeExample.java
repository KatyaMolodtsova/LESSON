...
// статический импорт статического метода lastDayOfMonth из класса TemporalAdjusters
// позволяет вызвать метод без обращения к классу
...
public class DateTimeExample {
    public static void getLocalDateTimeDescription(){
        ...
        System.out.println(LocalDateTime.now().getDayOfWeek()); // день недели и тд и тп
        System.out.println(LocalDateTime.now().plus(45, ChronoUnit.CENTURIES));
        System.out.println(LocalDateTime.now().minus(2, ChronoUnit.DAYS));

        System.out.println(MonthDay.from(LocalDateTime.now())); // месяц от даты
        System.out.println(DayOfWeek.from(LocalDateTime.now())); // день недели от даты

        // принимает на вход тип данных TemporalAdjuster
        System.out.println(LocalDateTime.now().with(TemporalAdjusters.next(DayOfWeek.SATURDAY)));
        System.out.println(LocalDateTime.now().with(some));
