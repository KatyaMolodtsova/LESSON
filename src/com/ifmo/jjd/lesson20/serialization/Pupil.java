...
public class Pupil extends Human implements LearnAble {
    private Group group; // если ссылки на классы, то тоже должны быть Serializable
    private int level;
    private LocalDate lastLesson;
    transient private final String info = "Ученик"; // исключили из процесса сериализации
    // либо которые мешаются, либо вычисляемые

    // уникальный идентификатор версии сериализованного объекта, записывается в потом при сериализации
    // при десериализации сравниваются значение этого поля с имеющимся у локального класса
    // рекомендуется прописывать версию, если имплементирует интерфейс Serializable
